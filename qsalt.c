...
ZK up(K x){memmove(xG,xG+32,xn-32);xn-=32;while(xn&&!xG[xn-1])xn--;R x;}ZI kn(K x){R xt>=0?xn:-1;} ZK gn(V){R kpn(KG,0);}
...
ZK pencrypt(K x,K y,K z){K m,r;L(y,32);L(z,32);m=gp(xy,32),r=ktn(KG,m->n);crypto_box_curve25519xsalsa20poly1305_ref(kG(r),kG(m),m->n,kG(xx),kG(y),kG(z));r0(m);jv(&r,xx);R r;}//(nonce;text);receiverpublic;sendersecret->message
ZK pdecrypt(K x,K y,K z){K r;L(y,32);L(z,32);P(xn<64,gn()); r=ktn(KG,xn);if(-1==crypto_box_curve25519xsalsa20poly1305_ref_open(kG(r),xG,xn-32,xG+(xn-32),kG(y),kG(z)))r0(r),r=gn();else r=up(r);R r;} //message;senderpublic;receiversecret->text
...
ZK sign(K x,K y){K r;j n;L(y,64);r=ktn(KG,xn+64);n=r->n;crypto_sign_edwards25519sha512batch_ref(kG(r),&n,xG,xn,kG(y));r->n=n;R r;}//text;secretkey->message
ZK verify(K x,K y){K r;j n;L(y,32);P(xn<64,gn()); r=ktn(KG,xn-64);n=r->n;if(-1==crypto_sign_edwards25519sha512batch_ref_open(kG(r),&n,xG,xn,kG(y))){r0(r);R gn();}r->n=n;R r;}//message;publickey->text|null
...
ZK sdecrypt(K x,K y){K r;P(xn<32,gn()); r=ktn(KG,xn);if(-1==crypto_secretbox_xsalsa20poly1305_ref_open(kG(r),xG,xn-32,xG+(xn-32),kG(y)))r0(r),r=gn();else r=up(r);R r;}//message;secret->text
