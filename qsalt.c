...
ZK pkey(K x){G p[32];crypto_box_curve25519xsalsa20poly1305_ref_keypair(p,xG);R gg(kpn(p,32));}//secret->public
...
ZK pdecrypt(K x,K y,K z){K r=ktn(KG,xn-32);if(-1==crypto_box_curve25519xsalsa20poly1305_ref_open(kG(r),xG,xn-32,xG+(xn-32),kG(y),kG(z))){r0(r);R gg(ks(""));}R r;}//message;senderpublic;receiversecret->text

ZK newkey(K _){G p[32],s[32];crypto_sign_edwards25519sha512batch_ref_keypair(p,s);R knk(2,gg(kpn(p,32)),gg(kpn(s,32)),0);}//->(public;secret)
...
ZK verify(K x,K y){K r=ktn(KG,xn-64);j n=r->n;if(-1==crypto_sign_edwards25519sha512batch_ref_open(kG(r),&n,xG,xn,kG(y))){r0(r);R gg(ks(""));}r->n=n;R r;}//message;publickey->text|null
...
ZK sdecrypt(K x,K y){K r=ktn(KG,xn-32);if(-1==crypto_secretbox_xsalsa20poly1305_ref_open(kG(r),xG,xn-32,xG+(xn-32),kG(y))){r0(r);R gg(ks(""));}R r;}//message;secret->text
